...
12. `sources.py` — **what a deck may load.** `SourcePolicy.check()` says whether a picture's address may be read: a saved deck reads its own folder and the installed theme folders; an unsaved draft (no folder) may read local files anywhere, because the writer put every picture there; the web only when the frontmatter says `remote_images: true`; `data:` always. `html._screen_images()` blanks every picture the policy refuses before Pillow, the size check or WeasyPrint can touch it, and records it in `slide_info` as `outside_images` or `remote_images` for the banner. `SourcePolicy.fetcher()` is handed to **every** `weasyprint.HTML(...)` call — the build, the live render, the CLI, the handout (`data_only()`) and the theme thumbnails — so a theme's `url()` or an SVG cannot fetch past the check. When a draft is saved for the first time, `Editor.adopt_outside_pictures()` copies its outside pictures into `assets/` so they still load.
13. `handout.py` — the talk as a document: each slide's picture with the `^^^` script beneath it, images inlined as data URIs. Deliberately unthemed — a theme is display type for a room, a handout is read at arm's length.
...
- `export_controller.py` — the four export flows (PDF / HTML / images / handout) behind one `_ask_save_path()`: dialog, suffix defaulting, writability check. PDF export re-points the build; the other three consume one via `builds.with_current_build()`. When the deck reveals in steps, Export PDF first asks "Every step" or "One page per slide"; the second is `converter.export_slides_pdf_async()`, a separate `reveal=False` render that never touches the build's state, so Present keeps its steps.

**A build writes the PDF and nothing else beside it.** The HTML a build needs on the way goes to one cache file per converter (`converter._html_output()`, deleted by `discard_html_output()` when the window closes), and the CLI writes no HTML at all. It used to go next to the output as `talk.html`, replacing whatever file of that name the writer had — and Export HTML's suggested name was that same file.

**Every deck PDF is tagged.** `pagination.slide_pages_pdf()` writes with `pdf_tags=True` — a write option in WeasyPrint 68, not a render one — so reading order and each picture's alt text (its description, from `image_attrs.description_of()`, with retired alt-text tokens left out) reach the PDF, as do `<title>` and the author from the frontmatter. `test_accessible_exports.py` reads them back with Poppler.

**The Flatpak runs nothing on the host.** Open PDF goes through `Gtk.FileLauncher` (the OpenURI portal) and falls back to opening the folder. The manifest used to grant `--talk-name=org.freedesktop.Flatpak` for a `flatpak-spawn --host xdg-open`, which is a full sandbox escape; `test_sandbox_escape.py` keeps both out.
