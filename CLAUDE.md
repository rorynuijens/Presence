...
- `sidebar.py` — thumbnail strip. Two sources feed it and they are kept apart on purpose: everything the document alone can answer — title, number, script indicator, per-slide timing — is read out of the text by `read_slides()` on every pause in typing, so it is never a build behind; only the picture and the overflow badge need a build. Pictures are matched to slides by content, never by index (`carry_over()`), and a row whose own slide has changed since the build is marked out of date rather than showing a stale picture silently. Three sizes are chained and must stay that way: `THUMBNAIL_WIDTH` is what a row displays, `SIDEBAR_WIDTH` is that plus the row margins (the window's `max_sidebar_width` reads it), and `thumbnails_render.THUMB_W` is twice it, because a picture shown at 320 logical pixels needs 640 device pixels on a 2x display.
...
- `build_coordinator.py` — when a build runs, what the header chip says, and what runs once it lands (`with_current_build`). Also owns fold lines, merging the build's answer for every slide with the canvas's fresher one for the slide being edited, and the waits held by whoever asked for the build (`wait_for_build`), released together on success and on failure.
