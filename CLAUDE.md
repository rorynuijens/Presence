...
**The seam is written down.** Each controller declares a Protocol for the window it takes — `BuildHost` (7 names), `DocumentHost` (14), `ExportHost` (7) — at the top of its own module, so what it needs is next to what needs it. `tests/test_host_protocols.py` checks both directions: everything a Host promises, `MainWindow` provides; everything a controller reaches for, its Host declares. The second direction is the one that stops the seam growing back, one perfectly reasonable attribute at a time.
