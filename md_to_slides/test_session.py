...
def tmp_session(monkeypatch, tmp_path):
    ...
    import md_to_slides.session as s
...
def test_save_load_last_file(tmp_path):
    from md_to_slides.session import save_last_file, load_last_file
    p = tmp_path / "test.md"
    p.write_text("# Hello")
    save_last_file(p)
...
def test_load_last_file_missing_returns_none():
    from md_to_slides.session import load_last_file
    assert load_last_file() is None


def test_save_load_window_state():
    from md_to_slides.session import save_window_state, load_window_state
...
def test_window_state_defaults():
    from md_to_slides.session import load_window_state
    s = load_window_state()
    assert s["width"] == 1400
    assert s["height"] == 860
...
def test_save_load_editor_prefs():
    from md_to_slides.session import save_editor_prefs, load_editor_prefs
    save_editor_prefs({
...
def test_editor_prefs_defaults():
    from md_to_slides.session import load_editor_prefs
    p = load_editor_prefs()
...
def test_save_load_presentation_prefs():
    from md_to_slides.session import save_presentation_prefs, load_presentation_prefs
...
def test_presentation_prefs_all_keys_in_fallback():
    ...
    from md_to_slides.session import load_presentation_prefs
    p = load_presentation_prefs()
...
def test_recent_files_round_trip(tmp_path):
    from md_to_slides.session import save_recent_file, load_recent_files
...
def test_recovery_path_deterministic(tmp_path):
    from md_to_slides.session import recovery_path_for
    p = tmp_path / "pres.md"
